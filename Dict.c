...
// Print words in tree using preorder traversal
static void preorder (Link n);
// Insert and sort into array
static void arrayInsert (Link n, WFreq *wfs, int max, int count);
// Helper function for arrrayInsert
static void orderInsert(WFreq *wfs, int max, WFreq *value);
...
{
   int counter = 0;
   arrayInsert(d->tree, wfs, n, counter);
   return 0; // Returns 0 as returned value not used in tw.c
}
...
static void arrayInsert (Link n, WFreq *wfs, int max, int count) {
   ...
   WFreq *ptr = &n->data;
   if (count < max) {
      orderInsert(wfs, max, ptr);
   } else {
      if (n->data.freq > wfs[max - 1].freq) {
         orderInsert(wfs, max, ptr);
      } else if (wfs[max - 1].freq == n->data.freq) {
         if (strcmp(n->data.word, wfs[max - 1].word) > 0) {
            orderInsert(wfs, max, ptr);
         }
      }
   }
   count++;
   arrayInsert(n->right, wfs, max, count);
   arrayInsert(n->left, wfs, max, count);
}

static void orderInsert(WFreq *wfs, int max, WFreq *value) {
   wfs[max - 1] = *value;
   int counter = max - 1;
   if (value->freq >= wfs[counter].freq) {
      wfs[counter] = *value;
      counter--;
   } 
   while (counter >= 0 && value->freq >= wfs[counter].freq) {
      if (value->freq == wfs[counter].freq) {
         if (strcmp(value->word, wfs[counter].word) > 0) {
            break;
         }
      }
      wfs[counter + 1] = wfs[counter];
      wfs[counter] = *value;
      counter--;
   }
}
...
static Link rotateRight(Link n) {
   ...
   if (n->left == NULL) return n;
      ...
      newRoot->height = roof(height(newRoot->left), 
